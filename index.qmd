...
The correlation matrix reveals that alcohol (r = 0.44) and density (r =
–0.31) have the strongest linear relationships with wine quality. In
contrast, sulphates shows only a weak correlation (r = 0.04). While
correlation is useful for identifying direct, linear associations, it
does not capture complex interactions between features. This is a key
limitation, as some variables may appear weakly correlated but still
play an important role when combined with others. For example, sulphates
may influence wine quality in non-linear or conditional ways that are
not reflected in its standalone correlation.
